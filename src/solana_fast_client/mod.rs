use {
    ...
    napi::{Result,
           threadsafe_function::ErrorStrategy::CalleeHandled,
...
#[napi(js_name = "SolanaFastClient")]
pub struct SolanaFastClient {
    rpc_url: String,
    ws_url: String,
    tpu_client: Option<TpuClient<QuicPool, QuicConnectionManager, QuicConfig>>,
    pub_sub_client: Arc<Option<PubsubClient>>,
}

impl Deref for SolanaFastClient {
...
async fn create_tpu_client(
    ...
    TpuClient::<QuicPool, QuicConnectionManager, QuicConfig>::new(
        ...
        .map_err(|e| {
            napi::Error::new(
                napi::Status::Unknown,
                format!("SolanaFastClient::connect error: {e:?}"),
            )
        })
}
...
async fn test_on_program_account_change() {
    ...
    pub async fn on_program_account_change(pubkey: String) -> napi::Result<()> {
        ...
        let p_client =
            PubsubClient::new("wss://api.mainnet-beta.solana.com")
                .await
                .unwrap();
